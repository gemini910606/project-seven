...
namespace Game.Bots
{
    ...
    public sealed class NoiseSystem : MonoBehaviour
    {
        ...
        private readonly List<BotPerception> _listeners = new();
        ...
        public void Register(BotPerception listener)
        {
            if (listener != null && !_listeners.Contains(listener)) _listeners.Add(listener);
        }

        public void Unregister(BotPerception listener) => _listeners.Remove(listener);
        ...
        public void Emit(Vector3 position, float radius)
        {
            ...
            {
                BotPerception listener = _listeners[i];
                if (listener == null)
                {
